...
var filteredJobs = function() {
	var category = Router.current().params.category;
    return Jobs.find({$and: [{invited: false}, {$or: [{applicationStatus: 'open'}, {$and: [{applicationStatus: 'assigned'}, {assignmentStatus: 'not_confirmed'}]}]}, {jobtype: {$in: [category]}}]}).fetch();
}

var filteredJobsOptions = {
...
Template.filteredJobs.helpers({
    ...
    filteredJobsOptions: filteredJobsOptions
})