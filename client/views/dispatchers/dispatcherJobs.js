var dispatcherJobs = function() {
	var dispatcherId = Dispatchers.findOne({_id: Router.current().params._id}).userId;
    return Jobs.find({$and: [{invited: false}, {routed: false}, {status: 'active'}, {$or: [{applicationStatus: 'open'}, {$and: [{applicationStatus: 'assigned'}, {assignmentStatus: 'not_confirmed'}]}]}, {userId: dispatcherId}]}).fetch();
}
...
Template.dispatcherJobs.helpers({
    ...
    dispatcherJobsOptions: buyerJobsOptions
});